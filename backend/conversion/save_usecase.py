...
class UsecaseSaver():
  ...
  def save_event(self, model: pyd.Event, uc_id):
    ...
    newEvent, _ = djg.Event.objects.update_or_create(name=model.name, usecase_id=self.usecase_map[uc_id], defaults={"actor_id": self.actor_map[model.actor.id], "description": model.description})
  ...
  #       related_classes=[self.class_map[cls.id] for cls in model.related_classes],
  #       next_step_id=None)
