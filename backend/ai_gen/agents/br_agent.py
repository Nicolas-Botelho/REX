...
from backend.ai_gen.response_model.requirement_response import BROutput

from langchain_core.messages import BaseMessage, AIMessage, HumanMessage, SystemMessage
from typing import List

def call_llm(state: State):
  print('BR Agent Start')

  input_text = state.get('InputText')
  new_fr = state.get('FunctionalRequirements')
  new_nfr = state.get('NonFunctionalRequirements')

  llm_struct_output = llm.with_structured_output(BROutput)

  instruction = f'''
  # Task
  You are a Requirements Engineer tasked with extracting the business rules (BR) of a given text about a system and given the functional requirements (FR) of that system.

  ## About the Requirements
  - Only extract explicit and implied requirements of the text, do not create any other requirements.
  - Use the Requirements Engineering techniques to identify the requirements of the text, even when there are more than one requirement in the same part of the text (e.g.: "The system should allow user registration by the Admin, provided that the user meets predefined eligibility criteria." means there is, beside the FR, a BR saying that all the users must meet the eligibility criteria.
  ...
  ## About the Structured Output Format for Business Rules (BR)
  A NFR have the following parts:
  - iD: A unique integer identification of the BR;
  ...
  - affectedFR: A list of all the FRs that are affected by that BR.
  ...
  if new_nfr:
    new_nfr_text = f'''
  ## New Non Functional Requirements (NFR)
  This are the new NFRs. Consider that this given requirements are 100% correct, so do not exclude or modify them. If a new requirement, created by you, contradict one of this requirements, change the new requirement, that is, the one that you created.

  ### Non Functional Requirements
  {new_nfr.__str__}
  '''

  llm_input = [SystemMessage(content=instruction+current_br_text), HumanMessage(content=input_text), HumanMessage(content=new_fr_text+new_nfr_text)]

  # supposedly, the response type should be the same of the structured output
  response = llm_struct_output.invoke(llm_input)
  # print(response.br_list)

  print('BR Agent End')

  return {**state, 'BusinessRules': response.br_list}